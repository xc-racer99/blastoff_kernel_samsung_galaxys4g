...
void tvout_ldo3_control(bool on)
{
	tv_csis = regulator_get(NULL,"usb_io");
	if(on)
	{
 		
        	regulator_enable(tv_csis);	//enable power here else s5p_tv_v_open will crash
	}
	else
	{
		regulator_disable(tv_csis);	//we call this turn off from tvout driver as after sii9234 turn off, hdmi core operation is still in progress
	}
